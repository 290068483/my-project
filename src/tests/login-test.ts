...
export async function testLoginAPI() {
  ...
  try {
    ...
    // 2. 测试captcha API的兼容性
    console.log("2. 测试captcha API兼容性...");
    const captchaResponse2 = await getCaptchaFromAPI();
    console.log("✅ Captcha API兼容性测试成功:", {
...
export function testUserStoreIntegration() {
  ...
  try {
    ...
    // 检查Store状态
    const storeState = ["token", "userInfo", "isLoggedIn", "captchaUuid", "captchaImage", "permissions", "roles"];

    console.log("✅ 用户Store状态字段:", storeState);
...
export async function runAllTests() {
  ...
  if (allPassed) {
    ...
    console.log("  • 实现完整的路由权限控制");
    console.log("  • 优化用户状态管理");
    console.log("  • 增强验证码机制");
    console.log("  • 改进错误处理和用户体验");
  }
...
if (typeof window !== "undefined") {
  // 浏览器环境
  (window as typeof window & { testRuoYiLogin: Function }).testRuoYiLogin = runAllTests;
  console.log("💡 提示: 在浏览器控制台中输入 testRuoYiLogin() 来运行测试");
}
