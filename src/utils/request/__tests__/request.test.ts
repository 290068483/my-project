...
describe("Request Module", () => {
  ...
  describe("File Operations", () => {
    ...
    it("should validate file size during upload", async () => {
      ...
      await expect(request.upload("/upload", mockFile, { maxSize: 100000 })).rejects.toThrow("文件大小超过限制");
    });

    it("should validate file type during upload", async () => {
      const mockFile = new File(["file content"], "test.txt", { type: "text/plain" });

      await expect(request.upload("/upload", mockFile, { accept: ".jpg,.png" })).rejects.toThrow("不支持的文件类型");
    });
  });
