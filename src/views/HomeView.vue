<template>
  <div
    class="home-container min-h-screen min-w-full bg-neutral flex flex-col font-sans"
  >
    <main class="w-full px-4 sm:px-6 pt-6 sm:pt-8 pb-4 sm:pb-6 flex-grow">
      ...
      >
        <div
          class="user-details flex items-center space-x-3 sm:space-x-4 mb-2 sm:mb-0 w-full sm:w-auto"
        >
          <div
        ...
        <div class="notifications flex flex-wrap gap-3">
          ...
          >
            ...
            <p class="text-xs text-gray-700">
              <span class="font-semibold">活动通知：</span>
              6月12日项目评审会议，请准时参加。
            </p>
          </div>
          <div
      ...
      <!-- 快捷信息区域 -->
      <div
        class="quick-info mb-6 grid grid-cols-1 md:grid-cols-3 gap-4 bg-white p-4 rounded-lg shadow-card"
      >
        <div
      ...
      <div class="data-tables grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        >
          ...
          >
            ...
            <button
              class="text-xs text-white hover:text-blue-200 flex items-center"
            >
              <span>详情</span>
              <svg
                class="w-3 h-3 ml-1"
        ...
        >
          <div
            class="table-header bg-blue-50 px-4 py-2 border-b border-blue-100 flex justify-between items-center"
          >
            <h3 class="text-sm font-semibold text-white">
              与本人关联的所有事项
            </h3>
          ...
          <template v-for="(announcement, index) in announcements" :key="index">
            ...
            >
              <p class="text-sm text-gray-700 mb-1 relative">
                <span
                  class="absolute left-0 top-1/2 transform -translate-y-1/2 w-2 h-2 bg-blue-500 rounded-full mr-2"
                ></span>
                <span class="ml-4 font-medium">{{ announcement.title }}</span
...
const announcements = ref<Announcement[]>([
  {
    title: "3月24日会议通知：",
    content:
      "3月24日合同，谢永辉合同，合同款32465元，有2000元尾款，请送货时切记收取。",
  },
  {
    title: "3月24日合同：",
  ...
  },
  {
    title: "3月24日通知：",
    content:
      "潘道国已完成客户测量，明天上午上门测量，请及时与客户联系，约定具体时间。",
  },
]);
